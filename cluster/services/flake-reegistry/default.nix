...
{
  ways = config.lib.forService "flake-registry" {
    registry.static = { depot, pkgs, ... }: pkgs.writeTextDir "flake-registry.json" (let
      flakes = {
        depot = {
          type = "tarball";
        ...
        depot-nixpkgs = {
          ...
          owner = "NixOS";
          repo = "nixpkgs";
          inherit (depot.inputs.nixpkgs-input.sourceInfo) rev narHash lastModified;
        };
        blank = {
          type = "github";
          owner = "divnix";
